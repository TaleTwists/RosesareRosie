...
const BottomNavigation: FC<BottomNavigationProps> = ({ customNavItems, className }) => {
  ...
  const renderLucideIcon = (iconType: 'house' | 'shopping-cart' | 'user', active: boolean) => {
  ...
  return (
    <nav className={`fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 safe-area-pb z-50 ${className || ''}`}>
        ...
        {navItems.map((item: NavItem) => {
          ...
          if (item.useAccountHandler && user) {
            return (
              ...
              >
                <div className={`relative w-6 h-6 mb-1 transition-transform duration-200 flex items-center justify-center ${
