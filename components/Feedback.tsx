...
const Feedback: React.FC<RecentFeedbackProps> = ({ 
      ...
      <div className="flex gap-1">
        {[1, 2, 3, 4, 5].map((star) => (
          <Star
            key={star}
            className={`w-5 h-5 ${
              star <= rating ? 'fill-shop_light_green text-shop_light_green' : 'text-gray-300'
            }`}
          />
        ))}
      </div>
  ...
  return (
    <div className="relative min-h-screen bg-white py-12 px-4">
      <div className={`relative max-w-4xl mx-auto bg-white rounded-2xl p-8 shadow-sm overflow-hidden ${className}`}>
        {/* Scattered Icons Background - Now inside the component */}
